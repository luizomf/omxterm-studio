...
- Prefer one responsible implementer and a fresh independent reviewer. Inherit the active conversation's model and reasoning unless the maintainer explicitly requests another route; reuse that choice throughout the task without per-role mapping questions or repeated confirmation. Independent review requires a fresh conversation, not a different model. Obey harness routing rules.
